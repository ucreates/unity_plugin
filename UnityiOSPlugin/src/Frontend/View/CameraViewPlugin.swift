...
open class CameraViewPlugin: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    open func create(gameObjectName: String, onShowCallbackName: String, onHideCallbackName: String) -> Void {
        ...
        func callback(granted: Bool) -> Void {
            ...
            if #available(iOS 10.0, *) {
                var deviceTypes: [AVCaptureDevice.DeviceType] = [AVCaptureDevice.DeviceType]()
                ...
                let deviceSession: AVCaptureDevice.DiscoverySession = AVCaptureDevice.DiscoverySession(deviceTypes: deviceTypes, mediaType: AVMediaType.video, position: AVCaptureDevice.Position.unspecified)
                deviceList = deviceSession.devices
            } else {
                deviceList = AVCaptureDevice.devices(for: AVMediaType.video) as [AnyObject]
            }
            ...
            self.output.videoSettings = [kCVPixelBufferPixelFormatTypeKey as AnyHashable: Int(kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange)] as? [String : Any]
            ...
            for connectionObject: Any in self.output.connections {
                ...
                for portObject: Any in connection.inputPorts {
                    let port: AVCaptureInput.Port = portObject as! AVCaptureInput.Port
        ...
        if (status != AVAuthorizationStatus.authorized) {
            AVCaptureDevice.requestAccess(for: AVMediaType.video, completionHandler: callback)
        } else {
    ...
    public func captureOutput(_ captureOutput: AVCaptureOutput!, didOutput sampleBuffer: CMSampleBuffer!, from connection: AVCaptureConnection!) -> Void {
